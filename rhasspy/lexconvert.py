...
def LexFormats():
    ...
    return {
        "festival": makeDic(
            ...
            ("z", z),
            ("zh", ge_of_blige_etc),
            lex_filename=ifset("HOME", os.environ.get("HOME", "") + os.sep)
            + ".festivalrc",
        ...
        "espeak": makeDic(
            ...
            (
                ...
                a_as_in_apple,
                False,
            ),  # TODO: this is actually an a_as_in_apple variant in espeak; festival @1 is not in mrpa PhoneSet
            ("&", a_as_in_apple, False),
            ("V", u_as_in_but),
            ("0", o_as_in_orange),
            ("aU", o_as_in_now),
            ...
            (
                ...
                a_as_in_ago,
                False,
            ),  # (TODO: eSpeak sometimes uses a# in 'had' when in a sentence, and this doesn't always sound good on other synths; might sometimes want to convert it to a_as_in_apple; not sure what contexts would call for this though)
            ("3:", e_as_in_herd),
            ("3", var1_a_as_in_ago),
            ("@2", a_as_in_ago, False),
            (
                ...
                a_as_in_ago,
                False,
            ),  # (eSpeak @- sounds to me like a shorter version of @, TODO: double-check the relationship between @ and @2 in Festival)
            ("aI", eye),
            ("aI2", eye, False),
            ("aI;", eye, False),
            ("aI2;", eye, False),
            ...
            (
                "oUl",
                opt_ol_as_in_gold,
            ),  # (espeak says "gold" in a slightly 'posh' way though) (if dest format doesn't have opt_ol_as_in_gold, it'll get o_as_in_go + the l)
            ("OI", oy_as_in_toy),
            ("p", p),
            ("r", r),
            ("r-", r, False),
            ...
            (
                "Ul",
                opt_ul_as_in_pull,
            ),  # if dest format doesn't have this, it'll get opt_u_as_in_pull from the U, then the l
            ("u:", oo_as_in_food),
            ("O:", close_to_or),
            ("O@", var3_close_to_or),
            ...
            lex_read_function=lambda lexfile: [
                x
                for x in [l.split()[:2] for l in lexfile.readlines()]
                if len(x) == 2 and not "//" in x[0]
            ],
            lex_footer=lambda f: (
                f.close(),
                os.system("espeak --compile=en"),
            ),  # see also a bit of special-case code in mainopt_convert
            inline_format="[[%s]]",
            word_separator=" ",
            phoneme_separator="",
            ...
            cleanup_regexps=[
                ...
                (
                    "@U",
                    "oU",
                ),  # (eSpeak uses oU to represent @U; difference is given by its accent parameters)
                ("([iU]|([AO]:))@r$", "\1@"),
                ("([^e])@r", r"\1_remove_3"),
                ("_remove_", ""),
            ...
            cvtOut_regexps=[
                ("e@r$", "e@"),
                (
                    "e@r([bdDfghklmnNprsStTvwjzZ])",
                    r"e@\1",
                ),  # because the 'r' is implicit in other synths (but DO have it if there's another vowel to follow)
            ],
        ),
        "sapi": makeDic(
            ...
            (
                "h",
                h,
            ),  # Jan suggested 'hh', but I can't get this to work on Windows XP (TODO: try newer versions of Windows)
            ("ih", i_as_in_it),
            ("iy", e_as_in_eat),
            ("jh", j_as_in_jump),
            ("k", k),
        ...
        "cepstral": makeDic(
            ...
            lex_read_function=lambda lexfile: [
                (word, pronunc)
                for word, ignore, pronunc in [
                    l.split(None, 2) for l in lexfile.readlines()
                ]
            ],
            lex_word_case="lower",
        ...
        "mac-uk": makeDic(
            ...
            lex_read_function=lambda *args: [
                (w, p)
                for w, _, p in MacBritish_System_Lexicon(
                    False, os.environ.get("MACUK_VOICE", "Daniel")
                ).usable_words()
            ],
        ...
        "acapela-uk": makeDic(
            ...
            (".", syllable_separator),
            ('"', primary_stress),
            ("%", secondary_stress),  # copied from "x-sampa", not tested
            ("A:", a_as_in_ah),
            ("{", a_as_in_apple),
            ("V", u_as_in_but),
            ("Q", o_as_in_orange),
        ...
        "apollo": makeDic(
            ...
            (
                ...
                "",
                False,
            ),  # I don't think the Apollo had anything to mark stress; TODO: control the pitch instead like bbcmicro ?
        ...
        "dectalk": makeDic(
            ...
            cleanup_regexps=[
                ("yxuw", "yu")
            ],  # TODO: other allophones ("x',False" stuff above)?
        ...
        "doubletalk": makeDic(
            ...
            (
                "/",
                primary_stress,
            ),  # TODO: check it doesn't need a balancing \ afterwards (docs do say it's a "temporary" change of pitch, but it's unclear how long a 'temporary')
            ("M", m),
            ("N", n),
            ("NX", ng),
            ("O", o_as_in_go),
            ...
            (
                ...
                "O",
                False,
            ),  # TODO: is this the best approximation we can do?
            ("OY", oy_as_in_toy),
            ("P", p),
            ("R", r),
            ("S", s),
            ...
            ("L", l),
            ("PX", p, False),
            ("TX", t, False),  # aspirated allophones
            ("WH", w, False),
            ("KX", k, False),  # ditto
            ("YY", y),
            ("Y", y, False),
        ...
        "bbcmicro": makeDic(
            ...
            (
                "5",
                secondary_stress,
            ),  # (these are pitch numbers on the BBC; normal pitch is 6, and lower numbers are higher pitches, so try 5=secondary and 4=primary; 3 sounds less calm)
            ("AA", a_as_in_ah),
            ("AE", a_as_in_apple),
            ("AH", u_as_in_but),
            ("O", o_as_in_orange),
            ...
            (
                "IX",
                var2_i_as_in_it,
            ),  # (IX sounds to me like a slightly shorter version of IH)
            ("IXAH", ear),
            ...
            (
                "OL",
                opt_ol_as_in_gold,
            ),  # (if dest format doesn't have this, it'll get o_as_in_orange from the O, then the l)
            ("OY", oy_as_in_toy),
            ("P", p),
            ("R", r),
            ("S", s),
            ...
            (
                ...
                oor_as_in_poor,
                False,
            ),  # TODO: really? (espeak 'U' goes to opt_u_as_in_pull, and eSpeak also used U for the o in good, which sounds best with Speech's default UH4, hence the line below, but where did we get UH->oor_as_in_poor from?  Low-priority though because how often do you convert OUT of bbcmicro format)
            (opt_u_as_in_pull, "UH", False),
            ("/U", opt_u_as_in_pull, False),
            (
                "/UL",
                opt_ul_as_in_pull,
            ),  # if dest format doesn't have this, it'll get opt_u_as_in_pull from the /U, then l
            ("UW", oo_as_in_food),
            ...
            lex_entry_format="> %s_"
            + chr(128)
            + "%s",  # (specifying 'whole word' for now; remove the space before or the _ after if you want)
            lex_read_function=lambda lexfile: [
                (w[0].lstrip().rstrip("_").lower(), w[1])
                for w in [
                    x
                    for x in [w.split(chr(128)) for w in lexfile.read().split(">")]
                    if len(x) == 2
                ]
            ],  # TODO: this reads back the entries we generate, but is unlikely to work well with the wildcards in the default lexicon that would have been added if SPEECH_DISK was set (c.f. trying to read eSpeak's en_rules instead of en_extra)
            lex_word_case="upper",
        ...
        "speakjet": makeDic(
            ...
            (
                ...
                "",
                False,
            ),  # TODO: instead of having emphasis, the Speakjet has a 'faster' code for all NON-emphasized syllables
        ...
        "braille-ipa": makeDic(
            ...
            (
                "4>3",
                ar_as_in_year,
            ),  # (from \u025c\u02d0; TODO: check what happens to \u025d)
            (">5", a_as_in_air),
            (">#", var1_a_as_in_air),
            ("E3", var2_a_as_in_air),
            (">3", var3_a_as_in_air),
            ...
            ("2", glottal_stop),
            lex_filename=ifset(
                "BRAILLE_UNICODE", "words-ipa.txt", "words-ipa.brl"
            ),  # write-only for now
            lex_type="document",
            ...
            stress_comes_before_vowel=True,
            safe_to_drop_characters=True,  # TODO: really?
            cleanup_func=lambda r: ifset(
                "BRAILLE_UNICODE", ascii_braille_to_unicode, lambda x: x
            )(",7" + r + "7'"),
        ),
        "latex-ipa": makeDic(
        ...
        "pinyin-approx": makeDic(
            ...
            cleanup_regexps=[
                ...
                ("([ghklmnpstyz])[euo]0ya", r"\1a"),
                ("([ghklmnpstz])a([0-5]*)ne0", r"\1an\2"),
                ("([bdfghklmnpstwyz])[euo]0a([1-5])", r"\1a\2"),
                ("([bdjlmnpt])[euo]0yi", r"\1i"),
                ("([bjlmnp])i([1-5]*)ne0", r"\1in\2"),
                ("([zs])he0ei", r"\1hei"),
                ...
                ("yi([1-9])ne0", r"yin\1"),
                ("yu0", "yu5"),
                ("eng0", "eng5"),  # they won't work unvoiced anyway
        ...
        "kana-approx": makeDic(
            ...
            (
                secondary_stress,
                ifset("KANA_MORE_EMPH", "\\u30fc"),
                False,
            ),  # set KANA_MORE_EMPH environment variable if you want to try doubling the secondary-stressed vowels as well (doesn't always work very well; if it did, I'd put this line in a makeVariantDic called kana-approx-moreEmph or something)
            ...
            (
                ...
                "\\u3046\\u304a",
                False,
            ),  # TODO: \u3042, \u304a or \u3046 depending on the word?
            ...
            ("\\u3065", d),  # du (and so on)
            ("\\u3066\\u3085", th_as_in_think),
            (th_as_in_them, "\\u3066\\u3085", False),
            ("\\u3075", f),
            ("\\u3050", g),
            ("\\u306f", h),  # ha (as hu == fu)
            ("\\u3058\\u3085", j_as_in_jump),
            (ge_of_blige_etc, "\\u3058\\u3085", False),
            ("\\u304f", k),
            ("\\u308b", l),
            (r, "\\u308b", False),
            ("\\u3080", m),
            ...
            (
                v,
                ifset("KANA_V_AS_W", "\\u308f", "\\u3094"),
                False,
            ),  # TODO: document KANA_V_AS_W variable.  Is vu always supported? (it doesn't seem to show up in all fonts)
            ("\\u3086", y),
            ("\\u305a", z),
            ...
            cleanup_regexps=[
                ...
                (
                    "\\u3046\\u3043\\u3066\\u3085",
                    "\\u3046\\u3043\\u3065",
                ),  # sounds a bit better for words like 'with'
                (
                    "\\u3085\\u3046",
                    "\\u3085",
                ),  # and 'the' (especially with a_as_in_ago mapping to u'\u3046\u304a'; it's hard to get a convincing 'the' though, especially in isolation)
                (
                    "\\u3050\\u3050",
                    "\\u3050",
                ),  # gugu -> gu, sometimes comes up with 'gl-' combinations
                (
                    "\\u30fc\\u30fc+",
                    "\\u30fc",
                ),  # in case we put 30fc in the table AND a stress mark has been applied to it
                (
                    "^(.)$",
                    r"\1\u30fc",
                ),  # lengthen any word that ends up as a single kana (otherwise can be clipped badly)
                (
                    "^([\\u3042\\u3070\\u3060\\u304c\\u304b\\u3089\\u307e\\u306a\\u3071\\u3055\\u305f\\u3084\\u3056\\u308f]\\u3044)$",
                    r"\1\u30fc",
                ),  # ditto for -ai (TODO: -ao might need lengthening sometimes?? depends on context.  -ei, -oi, -ou seem OK)
            ],
 ...
 or: python lexconvert.py --try unicode-ipa '\\u02c8\\u0279\\u026adn\\u0329' (for Unicode put '\\uNNNN' or UTF-8)"""
    ...
    if not format in lexFormats:
        return (
            "No such format "
            + repr(format)
            + " (use --formats to see a list of formats)"
        )
    for phones in getInputText(i + 2, "phonemes in " + format + " format", "maybe"):
        ...
        os.popen("espeak -x", "w").write(
            markup_inline_word("espeak", espeak) + "\n"
...
Convert phonemes from <format> into Mac and try it using the Mac OS 'say' command"""
    ...
    for resp in getInputText(i + 2, "phonemes in " + format + " format", "maybe"):
        ...
        print(toSay)
        os.popen(macSayCommand() + " -v Vicki", "w").write(
            toSay
        )  # Need to specify a voice because the default voice might not be able to take Apple phonemes.  Vicki has been available since 10.3, as has the 'say' command (previous versions need osascript, see Gradint's code)
...
Set format to 'all' if you want to see the phonemes in ALL supported formats."""
    ...
    if not format in lexFormats and not format == "all":
        return (
            "No such format "
            + repr(format)
            + " (use --formats to see a list of formats)"
    ...
    for response in getInputText(i + 2, "text", "maybe"):
        response = pipeThroughEspeak(
            ...
            .replace("\\u02b9", "")
            .replace("\\u00b7", "")
        )  # (remove any 2032 and b7 pronunciation marks before passing to eSpeak)
        if not "\n" in response.rstrip() and "command" in response:
            return response.strip()  # 'bad cmd' / 'cmd not found'
        if format == "all":
            formats = sorted(k for k in list(lexFormats.keys()) if not k == "example")
        else:
            formats = [format]
        for format in formats:
            ...
            def out(doOneoff=True):
                if len(formats) > 1:
                    writeFormatHeader(format)
                if doOneoff:
                    sys.stdout.write(checkSetting(format, "inline_oneoff_header"))
                sys.stdout.write(checkSetting(format, "inline_header"))
                output_clauses(
                    format,
                    convert(
                        parseIntoWordsAndClauses("espeak", response), "espeak", format
                    ),
                )
            ...
            if os.environ.get("PHONES_PIPE_COMMAND", ""):
                o, sys.stdout = (
                    sys.stdout,
                    os.popen(os.environ["PHONES_PIPE_COMMAND"], "w"),
                )
                out()
                sys.stdout = o
...
You can optionally set the RUBY_GRADINT_CGI environment variable to the URL of an instance of Gradint Web Edition to generate audio links for each word.  If doing this in a Web Adjuster filter, see comments in the lexconvert source for setup details."""
    ...
    if format == "example":
        return (
            "The 'example' format cannot be used with --ruby; did you mean festival?"
        )  # as above
    elif format == "all":
        return (
            "The --phones all option cannot be used with --ruby"
        )  # (well you could implement it if you want but the resulting ruby would be quite unwieldy)
    if not format in lexFormats:
        return (
            "No such format "
            + repr(format)
    ...
    text = (
        getInputText(i + 2, "text")
        .replace("\\u2019".encode("utf-8"), "'")
        .replace("\\u2032".encode("utf-8"), "'")
        .replace("\\u00b4".encode("utf-8"), "'")
        .replace("\\u02b9".encode("utf-8"), "'")
        .replace("\\u00b7".encode("utf-8"), "")
        .replace("\\u00a0".encode("utf-8"), " ")
    )
    ...
    if "1.45." in vLine:
        ...
        for places in range(
            25
        ):  # TODO: really want unbounded, but (?<=...) is fixed-length
        ...
        for prevDigits in range(10):
            for beforeThat in [
                "^",
                r"[^.0-9,]",
            ]:  # beginning of string, or something OTHER than a decimal point / num
                wordRegexps.append(
                    r"(?<="
                    + beforeThat
                    + "[1-9]"
                    + "[0-9,]" * prevDigits
                    + r")(?<!,)(?<!000)(?# empty string )(?=(?:,?(?:[0-9]{3,3}))+(?:[^0-9]|$))"
                )
        ...
        wordRegexps.append(
            r"(?:0[1-9]|[1-9][0-9]?)(?=(?:,?(?:[0-9]{3,3}))*(?:[^0-9]|$))"
        )
        # 0 by itself (not preceded by digits) = "nought" :
        wordRegexps.append(r"(?<![0-9])0(?=[^0-9]|$)")
        wordRegexps.insert(
            0, "(?<=[^A-Za-z0-9_-])(?:of|on|in|that|with|for|was) (?:the|a)(?= )"
        )
        ...
        text2 = re.sub(
            r"\.?[0-9]+", "", text
        )  # unknown eSpeak version: don't annotate the numbers
    ...
    if gradint_cgi:
        linkStart, linkEnd = (
            ...
            + gradint_cgi
            + "?js=[["
            + w.replace("%", "%25").replace("&", "%26")
            + ']]&jsl=en" onclick="return h5a(this);">',
            "</a>",
        )
        print(
            r"""<script><!-- // HTML5-audio function
function h5a(link) {
...
//--></script>"""
    ...
    for start, end in sorted(list(matches.keys()), cmpFunc):
        ...
        if start == end:
            ...
            m = text[start:end].replace("&", "&amp;").replace("<", "&lt;")
        try:
            rt = rubyList.pop()
        except:
            rt = "ERROR"  # we've lost synchronisation
        if debug:
            title = " title=" + str(matches[(start, end)])
        else:
            title = ""
    ...
    while (
        rubyList
    ):  # oops, lost synchronisation the other way (TODO: show this per-paragraph? but don't call eSpeak too many times if processing many short paragraphs)
...
def pipeThroughEspeak(inpt):
    ...
    return subprocess.check_output(["espeak", "-q", "-x", inpt]).decode()
    # bufsize = (
    #     8192
    # )  # careful not to set this too big, as the OS might limit it (TODO can we check?)
    # ret = []
    ...
    #         if not splitAt:
    #             sys.stderr.write(
    #                 "Note: had to split eSpeak input and couldn't find a newline or space to do it on\n"
    #             )
    #             splitAt = bufsize
...
def espeak_version_line():
    return os.popen("espeak -h 2>&1").read().strip().split("\n")[0]
...
def mainopt_check_variants(i):
    ...
    for k, v in i:
        ...
        while True:
            ...
            if not eval(input("Again? 1/0: ")):
                break


def mainopt_check_for_similar_formats(i):
    # undocumented (won't appear in help text)
    items = list(lexFormats.items())
    r = []
    while items:
        k1, dic1 = items[0]
        for k2, dic2 in items[1:]:
            ...
            for kk, vv in list(dic1.items()):
                if not type(kk) == int:
                    continue
                if kk == syllable_separator:
                    continue
                if not dic2.get(kk, "!" + vv) == vv:
                    diff += 1
            r.append((diff, k1, k2))
        items = items[1:]
    r.sort()
    ...
    for diffs, format1, format2 in r:
        if format1 in had and format2 in had:
            continue
        had.add(format1)
        had.add(format2)
        if "names" in had:
...
def festival_group_stress(pronunc):
    ...
    groups = []
    thisGroup = [[], "0", False]  # phon,stress,complete
    ...
    return (
        "("
        + " ".join(("((" + " ".join(g[0]) + ") " + g[1] + ")") for g in groups)
        + ")"
    )
...
E.g.: python lexconvert.py --convert festival cepstral"""
    ...
    if fromFormat == toFormat:
        return "Cannot convert a lexicon to its own format (that could result in it being truncated)"
    if toFormat == "mac-uk":
        return "Cannot permanently save a Mac-UK lexicon; please use the --mac-uk option to read text"
    if toFormat == "example":
        return "Cannot overwrite the built-in example lexicon"
    for f in [fromFormat, toFormat]:
        if not f in lexFormats:
            return (
                "No such format "
                + repr(f)
                + " (use --formats to see a list of formats)"
    ...
    if not fname:
        return (
            "Write support for lexicons of format '%s' not yet implemented (need at least lex_filename and lex_entry_format); try using --phones or --phones2phones options instead"
            % (toFormat,)
        )
    if toFormat == "espeak":
        assert (
            fname == "en_extra"
        ), "If you changed eSpeak's lex_filename in the table you also need to change the code below"
        if os.system('mv en_extra en_extra~ && grep " // " en_extra~ > en_extra'):
            sys.stderr.write(
                "Warning: en_extra not found, making a new one\n(espeak compile will probably fail in this directory)\n"
        ...
        assert not l, (
            "File "
            + replHome(fname)
            + " already exists and is not empty; are you sure you want to overwrite it?  (Delete it first if so)"
        )  # (if you run with python -O then this is ignored, as are some other checks so be careful)
...
In all cases you need to cd to the eSpeak source directory before running this.  en_extra will be overwritten.  Converter will also read your ~/.festivalrc if it exists.  (You can later incrementally update from ~/.festivalrc using the --convert option; the entries from the system dictionary will not be overwritten in this case.)  Specify --without-check to bypass checking the existing eSpeak pronunciation for OALD entries (much faster, but makes a larger file and in some cases compromises the pronunciation quality)."""
    ...
    try:
        ...
        return (
            "Error: The specified OALD location '"
            + festival_location
            + "' could not be opened"
        )
    try:
    ...
    convert_system_festival_dictionary_to_espeak(
        festival_location,
        not "--without-check" in sys.argv,
    ...
    for txt in getInputText(i + 1, "word(s)", "maybe"):
        ...
        print(
            (
                " ".join(
                    [
                        hyphenate(word, sylcount(convert(line, "espeak", "example")))
                        for word, line in zip(words, [x for x in rrr if x])
                    ]
                )
            )
        )
...
def wordSeparator(format):
    ...
    return checkSetting(
        format, "word_separator", checkSetting(format, "phoneme_separator", " ")
    )
...
Perform a one-off conversion of phonemes from format1 to format2 (format2 can be 'all' if you want)"""  # If format1 is 'example' and you don't specify phonemes, we take the words from the example lexicon.  But don't say that in the help string because it might confuse the issue about phonemes being optional on the command line and prompted for if not specified and stdin is not piped in all formats other than 'example'.
    ...
    if not format1 in lexFormats:
        return (
            "No such format "
            + repr(format1)
            + " (use --formats to see a list of formats)"
        )
    if not format2 in lexFormats and not format2 == "all":
        return (
            "No such format "
            + repr(format2)
            + " (use --formats to see a list of formats)"
    ...
    if format1 == "example" and len(sys.argv) <= i + 3:
        if stdin_is_terminal():
            ...
            txt = sys.stdin.read()  # and it might still be ""
        if txt:
            parseIntoWordsAndClauses(format1, txt)
        else:
            clauses = [[x[1]] for x in getSetting("example", "lex_read_function")()]
    else:
        clauses = parseIntoWordsAndClauses(
            format1, getInputText(i + 3, "phonemes in " + format1 + " format")
        )
    if format2 == "all":
        formats = sorted(k for k in list(lexFormats.keys()) if not k == "example")
    else:
...
def parseIntoWordsAndClauses(format, phones):
    ...
    wordSep = checkSetting(
        format, "word_separator"
    )  # don't use wordSeparator() here - we're splitting, not joining, so we don't want it to default to phoneme_separator
    clauseSep = checkSetting(format, "clause_separator", "\n")

    def s(sep):
        if sep == " ":
            return None  # " " means ANY whitespace (TODO: document this?)
        else:
            return sep
    ...
    for i in range(len(clauses)):
        if wordSep:
            clauses[i] = clauses[i].split(s(wordSep))
        else:
            clauses[i] = [clauses[i]]
...
Speak text in Mac OS 10.7+ British voices while using a lexicon converted in from <from-format>. As these voices do not have user-modifiable lexicons, lexconvert must binary-patch your system's master lexicon; this is at your own risk! (Superuser privileges are needed the first time. A backup of the system file is made, and all changes are restored on normal exit but if you force-quit then you might need to restore the backup manually. Text speaking needs to be under lexconvert's control because it usually has to change the input words to make them fit the available space in the binary lexicon.) By default the Daniel voice is used; Emily or Serena can be selected by setting the MACUK_VOICE environment variable."""
    ...
    fromFormat = sys.argv[i + 1]
    if not fromFormat in lexFormats:
        return (
            "No such format "
            + repr(fromFormat)
            + " (use --formats to see a list of formats)"
...
def variant():
    ...
    Counter.sc += 1
    while str(Counter.sc).endswith("0"):
        Counter.sc += 1
...
def speakjet(symbol, opcode):
    ...
    if ifset("SPEAKJET_BINARY", 1):
        assert not ifset(
            "SPEAKJET_SYM", 1
        ), "Cannot set both SPEAKJET_SYM and SPEAKJET_BINARY"
        return chr(opcode)
    else:
        return ifset("SPEAKJET_SYM", symbol, str(opcode))
...
def makeDic(doc, *args, **kwargs):
    ...
    for a in args:
        ...
        if k in d:
            duplicates.add(k)
        v = a[1]
        assert (type(k) in [str, str] and type(v) in [int, float]) or (
            type(v) in [str, str] and type(k) in [int, float]
        ), (
            "Wrong types "
            + repr(a)
            + " (did you forget a _, before calling variant() or something?)"
        )
        d[k] = v
        if len(a) == 3:
            bidir = a[2]
    ...
    assert not duplicates, (
        ...
        + ", ".join(
            (
                repr(dup)
                + "".join(
                    " (=" + g + ")" for g, val in list(globals().items()) if val == dup
                )
            )
            for dup in sorted(list(duplicates))
        )
        + ". Did you forget a ,False to suppress bidirectional mapping?"
    )  # by the way, Python does not detect duplicate keys in {...} notation - it just lets you overwrite
    ...
    if missing and "approximate_missing" in kwargs:
        for miss, approxTo in [
            ...
            (
                a_as_in_ah,
                [a_as_in_apple],
            ),  # this seems to be missing in some American voices (DecTalk, Keynote, SAM); TODO: is this the best approximation we can do?
        ]:
            if miss in missing and all(x in d for x in approxTo):
                d[miss] = kwargs.get("phoneme_separator", " ").join(
                    d[x] for x in approxTo
                )
                # did_approx = True
                missing.remove(miss)
    ...
    if missing:
        ...
        sys.stderr.write(
            "WARNING: Some non-optional vowels/consonants are missing from "
            + repr(doc)
            + "\nThe following are missing: "
            + ", ".join(
                "/".join(g for g, val in list(globals().items()) if val == m)
                for m in missing
            )
            + "\n"
        )
    for k, v in list(kwargs.items()):
        d[("settings", k)] = v
    assert (
        type(d.get(("settings", "cleanup_regexps"), [])) == list
    ), "cleanup_regexps must be a list"  # not one tuple
    assert (
        type(d.get(("settings", "cvtOut_regexps"), [])) == list
    ), "cvtOut_regexps must be a list"  # not one tuple
    wsep = d.get(("settings", "word_separator"), None)
    psep = d.get(("settings", "phoneme_separator"), " ")
    if not wsep == None:
        assert not wsep in d, "word_separator duplicates with a key in " + repr(doc)
    if not psep == None:
        assert not psep in d, (
            "phoneme_separator duplicates with a key (did you forget to change the default, or to add a ,False somewhere?) in "
            + repr(doc)
        )
    global lastDictionaryMade
    lastDictionaryMade = d
    return d
...
def makeVariantDic(doc, *args, **kwargs):
    ...
    mainVowels, consonants = (
        [],
        [],
    )  # so makeDic doesn't complain if some vowels/consonants are missing
...
def make_dictionary(sourceName, destName):
    ...
    if (sourceName, destName) == (cached_sourceName, cached_destName):
        return cached_dict
    source = lexFormats[sourceName]
    dest = lexFormats[destName]
    d = {}
    global dest_consonants
    dest_consonants = set()
    global dest_syllable_sep
    dest_syllable_sep = dest.get(syllable_separator, "")
    ...
    for k, v in list(source.items()):
        ...
        if type(v) in [str, str]:
            continue  # (num->string entries are for converting IN to source; we want the string->num entries for converting out)
        if not v in dest:
            v = int(v)  # (try the main version of a variant)
        if not v in dest:
            continue  # (haven't got it - will have to ignore or break into parts)
        d[k] = dest[v]
        if int(v) in consonants:
            dest_consonants.add(d[k])
        if int(v) == e_as_in_herd and (
            not implicit_vowel_before_NL or v == int(v)
        ):  # TODO: or u_as_in_but ?  used by festival and some other synths before words ending 'n' or 'l' (see usage of implicit_vowel_before_NL later)
...
def convert(pronunc, source, dest):
    ...
    if source == dest:
        return (
            pronunc
        )  # essential for --try experimentation with codes not yet supported by lexconvert
        ...
        for lettersToTry in range(maxLen, -1, -1):
            if not lettersToTry:
                if safe_to_drop == True:
                    ...
                    (not safe_to_drop)
                    or not pronunc[0] in safe_to_drop
                    and not (pronunc[0], debugInfo) in warnedAlready
                ):
                ...
                isStressMark = toAdd and toAdd in [
                    lexFormats[dest].get(primary_stress, ""),
                    lexFormats[dest].get(secondary_stress, ""),
                ]
                if toAdd == lexFormats[dest].get(syllable_separator, ""):
                    pass
                elif isStressMark and not checkSetting(
                    dest, "stress_comes_before_vowel"
                ):
                    if checkSetting(source, "stress_comes_before_vowel"):
                        toAdd, toAddAfter = (
                            "",
                            toAdd,
                        )  # move stress marks from before vowel to after
                        ...
                        r = len(ret) - 1
                        while ret[r] in dest_consonants or ret[r].endswith("*added"):
                            r -= (
                                1
                            )  # (if that raises IndexError then the input had a stress mark before any vowel) ("*added" condition is there so that implicit vowels don't get the stress)
                        ret.insert(r + 1, toAdd)
                        toAdd = ""
                elif isStressMark and not checkSetting(
                    source, "stress_comes_before_vowel"
                ):  # it's a stress mark that should be moved from after the vowel to before it
                    i = len(ret)
                    while i and (
                        ret[i - 1] in dest_consonants or ret[i - 1].endswith("*added")
                    ):
                        i -= 1
                    if i:
                        i -= 1
                    ...
                    if dest_syllable_sep:
                        ret.append(
                            dest_syllable_sep
                        )  # (TODO: this assumes stress marks are at end of syllable rather than immediately after vowel; correct for Festival; check others; probably a harmless assumption though; mac-uk is better with syllable separators although espeak basically ignores them)
                    toAdd = ""
                # attempt to sort out the festival dictionary's (and other's) implicit_vowel_before_NL
                elif (
                    implicit_vowel_before_NL
                    and ret
                    and ret[-1]
                    and toAdd in ["n", "l"]
                    and ret[-1] in dest_consonants
                ):
                    ret.append(implicit_vowel_before_NL + "*added")
                elif (
                    len(ret) > 2
                    and ret[-2].endswith("*added")
                    and toAdd
                    and not toAdd in dest_consonants
                    and not toAdd == dest_syllable_sep
                ):
                    del ret[-2]
                if toAdd:
                    # Add it, but if toAdd is multiple phonemes, try to put toAddAfter after the FIRST phoneme
                    if separator:
                        toAdd = toAdd.split(separator)
                    else:
                        toAdd = [
                            toAdd
                        ]  # TODO: won't work for formats that don't have a phoneme separator (doesn't really matter for eSpeak though)
                    ret.append(toAdd[0])
    ...
    if ret and ret[-1] == dest_syllable_sep:
        del ret[-1]  # spurious syllable separator at end
...
def unicode_preprocess(pronunc):
    ...
    if (
        "\\u" in pronunc and not '"' in pronunc
    ):  # maybe \uNNNN copied from Gecko on X11, can just evaluate it to get the unicode
        # (NB make sure to quote the \'s if pasing in on the command line)
        try:
            pronunc = eval('u"' + pronunc + '"')
        except:
            pass
    else:  # see if it makes sense as utf-8
        try:
            pronunc = pronunc.decode("utf-8")
        except:
            pass
    return pronunc
    ...
    d = dict(
        list(
            zip(
                ...
                [chr(c) for c in range(0x2800, 0x2840)],
            )
        )
    )
...
def hiragana_to_katakana(u):
    ...
    assert type(u) == str
    if not os.environ.get("KANA_TYPE", "").lower().startswith("k"):
        return u
    u = list(u)
...
def espeak_probably_right_already(existing_pronunc, new_pronunc):
    ...
    if existing_pronunc == new_pronunc:
        return True

    def simplify(pronunc):
        return (
            pronunc.replace(";", "")
            .replace("%", "")
            .replace("a2", "@")
    ...
    if simplify(existing_pronunc) == simplify(new_pronunc):
        return (
            True
        )  # almost the same, and festival @/a2 etc seems to be a bit ambiguous so leave it alone
...
def parse_festival_dict(festival_location):
    ...
    for line in open(festival_location):
        ...
        if pos not in ["n", "v", "a", "cc", "dt", "in", "j", "k", "nil", "prp", "uh"]:
            continue  # two or more words
...
def convert_system_festival_dictionary_to_espeak(
    festival_location, check_existing_pronunciation, add_user_dictionary_also
):
    ...
    if check_existing_pronunciation:
        os.system(
            "espeak --compile=en"
        )  # so that the pronunciation we're checking against is not influenced by a previous version of en_extra
    ...
    for line in [
        x
        for x in open("en_list").read().split("\n")
        if x.split() and not re.match(r"^[a-z]* *\$", x)
    ]:
        ambiguous[line.split()[0]] = ambiguous[
            line.split()[0] + "s"
        ] = (
            True
        )  # this stops the code below from overriding anything already in espeak's en_list.  If taking out then you need to think carefully about words like "a", "the" etc.
    for word, pos, pronunc in parse_festival_dict(festival_location):
        pronunc = pronunc.replace("i@ 0 @ 0", "ii ou 2 ").replace(
            "i@ 0 u 0", "ii ou "
        )  # (hack for OALD's "radio"/"video"/"stereo"/"embryo" etc)
        pronunc = pronunc.replace(
            "0", ""
        )  # 0's not necessary, and OALD sometimes puts them in wrong places, confusing the converter
        if word in ["mosquitoes"]:
            continue  # OALD bug (TODO: any others?)
        if word in wordDic:
            ambiguous[word] = True
    ...
    for word, (pronunc, pos) in list(wordDic.items()):
        ...
        if not re.match(
            "^[A-Za-z]*$", word
        ):  # (some versions of eSpeak also OK with "-", but not all)
            # contains special characters - better not go there
            toDel.append(word)
        elif (
            word.startswith("plaque")
            or word in "friday saturday sunday tuesday thursday yesterday".split()
        ):
            ...
            toDel.append(word)
        elif word.startswith("year") or "quarter" in word:
            toDel.append(
                word
            )  # don't like festival's pronunciation of those (TODO: also 'memorial' why start with [m'I])
    ...
    items = list(wordDic.items())
    items.sort()  # necessary because of the hacks below which check for the presence of truncated versions of the word (want to have decided whether or not to output those truncated versions before reaching the hacks)
    for word, (pronunc, pos) in items:
        total_lines += 1
        new_e_pronunc = convert(pronunc, "festival", "espeak")
        if new_e_pronunc.count("'") == 2 and not "-" in word:
            new_e_pronunc = new_e_pronunc.replace(
                "'", ",", 1
            )  # if 2 primary accents then make the first one a secondary (except on hyphenated words)
        # TODO if not en-rp? - if (word.endswith("y") or word.endswith("ie")) and new_e_pronunc.endswith("i:"): new_e_pronunc=new_e_pronunc[:-2]+"I"
        unrelated_word = None
        if check_existing_pronunciation:
            espeakPronunc = oldPronDic.get(word, "")
        else:
            espeakPronunc = ""
        if word[-1] == "e" and word[:-1] in wordDic:
            unrelated_word, espeakPronunc = (
                word[:-1],
                "",
            )  # hack: if word ends with 'e' and dropping the 'e' leaves a valid word that's also in the dictionary, we DON'T want to drop this word on the grounds that espeak already gets it right, because if we do then adding 's' to this word may cause espeak to add 's' to the OTHER word ('-es' rule).
        if espeak_probably_right_already(espeakPronunc, new_e_pronunc):
            not_output_because_ok.append(word)
            continue
        if not unrelated_word:
            lines_output += 1
        outFile.write(
            word + " " + new_e_pronunc + " // from Festival's (" + pronunc + ")"
        )
        if espeakPronunc:
            outFile.write(", not [[" + espeakPronunc + "]]")
        elif unrelated_word:
            outFile.write(
                " (here to stop espeak's affix rules getting confused by Festival's \""
                + unrelated_word
                + '")'
            )
        outFile.write("\n")
    print(("Corrected(?) %d entries out of %d" % (lines_output, total_lines)))
    if add_user_dictionary_also:
        convert_user_lexicon("festival", "espeak", outFile)
    outFile.close()
    os.system("espeak --compile=en")
    if not_output_because_ok:
        print(
            "Checking for unwanted side-effects of those corrections"
        )  # e.g. terrible as Terr + ible, inducing as in+Duce+ing
        proc = os.popen("espeak -q -x -v en-rp > /tmp/.pronunc 2>&1", "w")
        progressCount = 0
        ...
        for word, pronunc in zip(
            not_output_because_ok, open("/tmp/.pronunc").read().split("\n")
        ):
            pronunc = pronunc.strip().replace(" ", "")
            if not pronunc == oldPronDic[word] and not espeak_probably_right_already(
                oldPronDic[word], pronunc
            ):
                outFile.write(
                    word
                    + " "
                    + oldPronDic[word]
                    + ' // (undo affix-side-effect from previous words that gave "'
                    + pronunc
                    + '")\n'
                )
        outFile.close()
...
def read_user_lexicon(fromFormat):
    ...
    if not readFunction:
        raise Message(
            "Reading from '%s' lexicon file not yet implemented (no lex_read_function); try using --phones or --phones2phones options instead"
            % (fromFormat,)
        )
    try:
        lexFilename = getSetting(fromFormat, "lex_filename")
        lexfile = open(lexFilename)
        if not os.environ.get("LEXCONVERT_OMIT_READING_FROM", ""):
            print(
                ("Reading from", lexFilename)
            )  # TODO: document LEXCONVERT_OMIT_READING_FROM (might be useful for the --mac-uk option)
    except KeyError:
        lexfile = (
            None
        )  # lex_read_function without lex_filename is allowed, if the read function can take null param and fetch the lexicon itself
    except IOError:
        raise Message(
            fromFormat
            + "'s lexicon is expected to be in a file called "
            + replHome(lexFilename)
            + " which could not be read - please fix and try again"
        )
...
def get_macuk_lexicon(fromFormat):
    ...
    return [
        (word, convert(pronunc, fromFormat, "mac-uk"))
        for word, pronunc in read_user_lexicon(fromFormat)
    ]
...
def convert_user_lexicon(fromFormat, toFormat, outFile):
    ...
    if type(lex_header) == str:
        outFile.write(lex_header)
    else:
        lex_header(outFile)
...
def markup_inline_word(format, pronunc):
    ...
    pronunc = as_utf8(
        pronunc
    )  # UTF-8 output - ok for pasting into Firefox etc *IF* the terminal/X11 understands utf-8 (otherwise redirect to a file, point the browser at it, and set encoding to utf-8, or try --convert'ing which will o/p HTML)
    format = checkSetting(format, "inline_format", "%s")
    if type(format) in [str, str]:
        # if type(format) == str:
        #     format = format.encode("utf-8")  # see above
...
def markup_bbcMicro_word(pronunc):
    ...
    if (not bbc_partsSoFar or bbc_partsSoFar + thisPartCount > 115) or (
        not bbc_charsSoFar or bbc_charsSoFar + len(pronunc) > 238
    ):  # 238 is max len of BBC BASIC prompt (both the immediate prompt and the one with line number supplied by AUTO, in both BASIC II and BASIC IV); re other limit see bbcMicro_partPhonemeCount
        if bbc_charsSoFar:
            r = "\n"
        else:
            r = ""
        cmd = (
            "*SPEAK"
        )  # (could add a space if want to make it more readable, at the expense of an extra keystroke in the paste buffer; by the way, when not using the ROM version you must use *SPEAK not OS.("SPEAK"), at least on a Model B; seems OSCLI doesn't go through quite the same vectors as star)
        bbc_charsSoFar = (
            len(cmd) + len(pronunc) + 1
        )  # +1 for the space that'll be after this word if we don't start a new line
...
def sylcount(example_format_festival):
    ...
    festival = (
        example_format_festival.split()
    )  # no brackets, emphasis by vowels, but spaces between each syllable
    for phone, i in zip(festival, list(range(len(festival)))):
        if phone[0] in "aeiou":
            inVowel = 0  # unconditionally start new syllable
        if phone[0] in "aeiou@12":
            if not inVowel:
                count += 1
            elif phone[0] == "@" and not hadAt:
                maybeCount = 1  # (e.g. "loyal", but NOT '1', e.g. "world")
            if "@" in phone:
                hadAt = 1  # for words like "cheerful" ("i@ 1 @" counts as one)
            inVowel = 1
            if (
                phone[0] == "@"
                and i >= 3
                and festival[i - 2 : i] == ["ai", "1"]
                and festival[i - 3] in ["s", "h"]
            ):  # special rule for higher, Messiah, etc - like "fire" but usually 2 syllables
                maybeCount = 0
                count += 1
        else:
            if not phone[0] in "drz":
                count += (
                    maybeCount
                )  # not 'r/z' e.g. "ours", "fired" usually 1 syllable in songs, "desirable" usually 4 not 5
...
def hyphenate(word, numSyls):
    ...
    orig = word
    try:
        word, isu8 = word.decode("utf-8"), True
    except:
        isu8 = False
    pre = []
    post = []
    ...
    while word and not "a" <= word[-1].lower() <= "z":
        post.insert(0, word[-1])
        word = word[:-1]
    if numSyls > len(word):
        return orig  # probably numbers or something
    l = int((len(word) + numSyls / 2) / numSyls)
    syls = []
    for i in range(numSyls):
        if i == numSyls - 1:
            syls.append(word[i * l :])
        else:
            syls.append(word[i * l : (i + 1) * l])
        if len(syls) > 1:
            if syls[-1].startswith("-") or (
                len(syls[-1]) > 2
                and syls[-1][0] == syls[-1][1]
                and not syls[-1][0].lower() in "aeiou"
            ):
                ...
                (
                    len(syls[-2]) > 2
                    and syls[-2][-1] == syls[-2][-2]
                    and not syls[-2][-1].lower() in "aeiou"
                )
...
def getInputText(i, prompt, as_iterable=False):
    ...
    if txt:
        if as_iterable == "maybe":
            ...
            return txt
    if stdin_is_terminal():
        sys.stderr.write("Enter " + prompt + " (EOF when done)\n")
    elif as_iterable == "maybe":
        return [sys.stdin.read()]
    if as_iterable:
        return my_xreadlines()
    else:
        try:
            return sys.stdin.read()
        except KeyboardInterrupt:
...
def output_clauses(format, clauses):
    ...
    if (
        checkSetting(format, "output_is_binary")
        and hasattr(sys.stdout, "isatty")
        and sys.stdout.isatty()
    ):
        print(
            "This is a binary format - not writing to terminal.\nPlease direct output to a file or pipe."
        )
        return
    clause_sep = checkSetting(format, "clause_separator", "\n")
    if type(clause_sep) in [str, str]:
        sys.stdout.write(
            clause_sep.join(
                wordSeparator(format).join(
                    markup_inline_word(format, word) for word in clause
                )
...
def print_bbc_warnings(keyCount, lineCount):
    ...
    for page, model in [
        (
            0x1900,
            "Model B",
        ),  # with Acorn DFS (a reasonable assumption although alternate DFS ROMs are different)
        (0xE00, "Master"),
    ]:  # (the Master has 8k of special paged-in "filing system RAM", so doesn't need 2816 bytes of main RAM for DFS)
        top = (
            page + keyCount + lineCount * (overhead_per_program_line - 1) + 2
        )  # the -1 is because keyCount includes a carriage return at the end of each line
        if model == "Master":
    ...
    if len(limits_exceeded) > 1:
        ...
        sys.stderr.write(
            warning
            + "this text may be too big for the BBC Micro because it exceeds the "
            + limits_exceeded[0]
            + after
        )
...
def bbc_prepDefaultLex(outFile):
    ...
    if not os.environ.get("MAKE_SPEECH_ROM", 0):
        return
    d = open(
        os.environ["SPEECH_DISK"]
    ).read()  # if this fails, SPEECH_DISK was not set or was set incorrectly (it's required for MAKE_SPEECH_ROM)
    i = d.index(
        "LO\x80LP\x80\x82\x11"
    )  # start of SP8000 file (if this fails, it wasn't a Speech disk)
...
def bbc_appendDefaultLex(outFile):
    ...
    if os.environ.get("SPEECH_DISK", ""):
        ...
        j = d.index(">**", i)
        assert j - i == 2201, (
            "Lexicon on SPEECH_DISK is wrong size (%d). Is this really an original disk image?"
            % (j - i)
        )
        outFile.write(d[i:j])
    ...
    assert not os.environ.get("MAKE_SPEECH_ROM", 0) or fileLen <= 16384, (
        "Speech ROM file got too big (%d)" % fileLen
    )
    outFile.close()
...
def bbcKeystrokes(data, start):
    ...
    use_int_hack = (start >= 1030 or start + len(data) <= 1026) and len(
        data.split(chr(128))
    ) >= 4
    i = 0
    ret = []
    if use_int_hack:
        thisLine = (
            "A%=&408:B%=&D80:"
        )  # (@% is at &400 and each is 4 byte LSB-MSB; $x reads to next 0D)
    ...
    while i < len(data):
        ...
        if data[i] == '"':
            c, inQ = (
                '""',
                1,
            )  # inQ MUST be 0 or 1, not False/True, because it's also used as 'len of necessary close quote' below
        elif 32 <= ord(data[i]) < 127:
            c, inQ = data[i], 1
        elif use_int_hack and ord(data[i]) == 128:
            c, inQ = "$A%", 0
        else:
            c, inQ = ("CHR$(" + str(ord(data[i])) + ")"), 0
        addToLine = []
        newNeedPlus = needPlus
        ...
        if (
            len(thisLine) + len(addToLine) + inQ > bbc_max_line_len
        ):  # oops, we've gone too far, back off and end prev line
            if inQuote:
                thisLine += '"'
            ret.append(thisLine)
            thisLine = ""
...
def print_bbclex_instructions(fname, size):
    ...
    if os.environ.get("MAKE_SPEECH_ROM", 0):
        print(
            (
                "%s (%d bytes, hex %X) can now installed on an emulator (set in Roms.cfg or whatever), or loaded onto a chip.  The sound quality of this might be worse than that of the main-RAM version."
                % (fname, size, size)
            )
        )  # (at least on emulation - see comment on sound quality above)
    else:
        print(
            ("The size of this lexicon is %d bytes (hex %X)" % (size, size))
        )  # (the default lexicon is 2204 bytes)
        bbcStart = None
        noSRAM_lex_offset = (
            0x155F
        )  # (on the BBC Micro, SRAM means Sideways RAM, not Static RAM as it does elsewhere; for clarity we'd better say "Sideways RAM" in all output)
        ...
        page = (
            0x1900
        )  # or 0xE00 for Master (but OK to just leave this at 0x1900 regardless of model; it harmlessly increases the range where special_relocate_instructions 'kick in')
        noSRAM_himem = (
            0x7C00
        )  # unless you're in a shadow mode or something (see comments on himem above), however leaving this at 0x7c00 is usually harmless (just causes the 'need to relocate' to 'kick in' earlier, although if memory is really full it might say 'too big' 1k too early)

        def special_relocate_instructions(reloc_addr):
            pagemove_min, pagemove_max = (
                max(0xE00, page - 0x1E00),
                page + 0xE00,
            )  # if relocating to within this range, must move PAGE before loading RELOCAT. RELOCAT's supported range is 0xE00 to 0x5E00, omitting (PAGE-&1E00) to (PAGE+&E00)
            if reloc_addr < 0x1900:
                extra = " On a Model B with Acorn DFS you won't be able to use the disk after relocating below &1900, and you can't run star commands from tape so you have to initialise via CALL. (On a Master, DFS is not affected as it doesn't use &E00-&1900.)"
            else:
                extra = ""
            ...
            if newpage > page_max:
                return (
                    False
                )  # "Unfortunately RELOCAT can't put it at &%X even with PAGE changes." % reloc_addr
            return (
                " Please run RELOCAT with PAGE in the range of &%X to &%X for this relocation to work.%s"
                % (newpage, page_max, extra)
            )
        ...
        if noSRAM_default_addr + noSRAM_lex_offset + size > noSRAM_himem:
            ...
            if reloc_addr >= noSRAM_min_addr:
                ...
                if instr == False:
                    print(
                        "This lexicon is too big for Speech in main RAM even with relocation, unless RELOCAT is rewritten to work from files."
                    )
                else:
                    ...
                    print(
                        (
                            "This lexicon is too big for Speech at its default address of &%X, but you could use RELOCAT to put a version at &%X and then initialise it with CALL %s (or do the suggested *SAVE, reset, and run *SP). Be sure to set HIMEM=&%X. Then *LOAD %s %X or change the relocated SP file from offset &%X.%s"
                            % (
                                noSRAM_default_addr,
                                reloc_addr,
                                bbcshortest(reloc_call),
                                reloc_addr,
                                fname,
                                bbcStart,
                                noSRAM_lex_offset,
                ...
                print(
                    "This lexicon is too big for Speech in main RAM even with relocation."
                )
        else:  # fits at default location - no relocation needed
            bbcStart = noSRAM_default_addr + noSRAM_lex_offset
            print(
                (
                    "You can load this lexicon by *LOAD %s %X or change the SPEECH file from offset &%X. Suggest you also set HIMEM=&%X for safety."
                    % (fname, bbcStart, noSRAM_lex_offset, noSRAM_default_addr)
                )
            )
        if bbcStart:  # we managed to fit it into main RAM
            ...
            print(
                (
                    "For ease of transfer to emulators etc, a self-contained keystroke file for putting %s data at &%X has been written to %s.key"
                    % (fname, bbcStart, fname)
                )
            )
            if len(keys) > 32767:
                print(
                    "(This file looks too big for BeebEm to paste though)"
                )  # see comments elsewhere
        # Instructions for replacing lex in SRAM:
        if size > SRAM_max - SRAM_lex_offset:
            print(
                "This lexicon is too big for Speech in Sideways RAM."
            )  # unless you can patch Speech to run in SRAM but read its lexicon from main RAM, or run in main RAM but page in multiple banks of SRAM for the lexicon (but even then there'll be a limit)
        else:
            print(
                (
                    "You can load this lexicon into Sideways RAM by *SRLOAD %s %X 7 (or whichever bank number you're using), or change the SP8000 file from offset &%X."
                    % (fname, SRAM_lex_offset + 0x8000, SRAM_lex_offset)
                )
            )
        if not os.environ.get("SPEECH_DISK", ""):
            print(
                "If you want to append the default lexicon to this one, set SPEECH_DISK to the image of the original Speech disk before running lexconvert, e.g. export SPEECH_DISK=/usr/local/BeebEm3/diskimg/Speech.ssd"
            )
        if size <= SRAM_max - SRAM_lex_offset:
            print(
                "You can also set MAKE_SPEECH_ROM=1 (along with SPEECH_DISK) to create a SPEECH.ROM file instead"
            )
    print(
        "If you get 'Mistake in speech' when testing some words, try starting with '*SAY, ' (this seems to be a Speech bug)"
    )  # - can't track down which words it does and doesn't apply to
    print(
        "It might be better to load your lexicon into eSpeak and use lexconvert's --phones option to drive the BBC with phonemes."
    )
...
def main():
    ...
    def funcToOpt(n):
        return "--" + n[n.index("_") + 1 :].replace("_", "-")
    ...
    )  # (undocumented option used for my website, don't rely on it staying)
    ...
    if (
        "--formats" in sys.argv
    ):  # non-HTML mode only (format descriptions are included in HTML anyway, and don't worry about the capability summary)
        print("Available pronunciation formats (and support levels):")
        keys = list(lexFormats.keys())
        keys.sort()
        for k in keys:
            types = []
            if not k == "example":
            ...
            if k == "mac-uk":
                ...
                if checkSetting(k, "lex_read_function"):
                    types.append("lex-read")
                if checkSetting(k, "lex_filename") and checkSetting(
                    k, "lex_entry_format"
                ):
    ...
    if html:
        print("<dl>")
    for _, opt, desc in [
        (v.__doc__ and not v.__doc__.startswith("*"), k, v.__doc__)
        for k, v in list(globals().items())
    ]:
        if not opt.startswith("mainopt_"):
            continue
        opt = funcToOpt(opt)
        if not desc:
            continue  # undocumented option
        ...
        if params:
            opt += " " + params
        if html:
            print(("<dt>" + htmlify(opt) + "</dt><dd>" + htmlify(rest) + "</dd>"))
        else:
            print((opt + "\n" + rest + "\n"))
    if html:
        print("</dl>")
...
class MacBritish_System_Lexicon(object):
    ...
    def __init__(self, text="", voice="Daniel"):
        ...
        self.voice = False
        if not text == False:
            assert not voice in MacBritish_System_Lexicon.instances, (
                "There is already another instance of MacBritish_System_Lexicon for the "
                + voice
                + " voice"
            )
            assert not os.system(
                "lockfile -1 -r 10 /tmp/" + voice + ".PCMWave.lock"
            )  # in case some other process has it (note: if you run with python -O, this check won't happen!)
            self.voice = (
                voice
            )  # (don't set this if text==False, since we won't need cleanup on __del__)
        self.filename = (
            "/System/Library/Speech/Voices/"
            + voice
            + ".SpeechVoice/Contents/Resources/PCMWave"
        )
        assert not (
            ...
            and os.path.exists(
                "/System/Library/Speech/Voices/"
                + voice
                + "Compact.SpeechVoice/Contents/Resources/PCMWave"
            )
        ), (
            "The only installation of "
            + voice
            + " found on this system was the Compact one, which lexconvert does not yet support"
        )  # TODO: could try self.wordIndexStart = findW("Abiquiu"),self.phIndexStart = findW("'@b.Ik.ju"),self.wordIndexEnd = findW("www.youtube.com",1),self.phIndexEnd = findW("'d^b.l.ju.'d^b.l.ju.'d^b.l.ju.dA+t.'ju.'tjub.dA+t.kA+m",1), but "t" in phones should be ignored, "activesync" and "afterlife" have no phones, "aqua" has TWO sets of phonemes (aquarium ok) and there are other synchronization issues.
        ...
        assert os.path.exists(self.filename), (
            "Cannot find an installation of '" + voice + "' on this system"
        )
        if os.path.exists(self.filename + "0"):
            if text == False:
                self.filename += (
                    "0"
                )  # (use the backup file for read-only, if we created one before; this means we don't have to worry about locks)
        elif not text == False:  # create a backup
            sys.stderr.write(
                "Backing up " + self.filename + " to " + self.filename + "0...\n"
        ...
        if os.path.exists(lexFile) and os.stat(lexFile).st_size:
            self.wordIndexStart, self.wordIndexEnd, self.phIndexStart, self.phIndexEnd = pickle.Unpickler(
                open(lexFile)
            ).load()
        else:
            ...
            def findW(word, rtnPastEnd=0):
                ...
                try:
                    ...
                    raise Exception(word + " not found in voice file" + compat_err)
                try:
                    n2 = next(i)
                    raise Exception(
                        "%s does not uniquely identify a byte position (has at least %d and %d)%s"
                        % (word, n.start(), n2.start(), compat_err)
                    )
                except StopIteration:
                    pass
                if rtnPastEnd:
            ...
            if not text == False:
                pickle.Pickler(open(lexFile, "w")).dump(
                    (
                        self.wordIndexStart,
                        self.wordIndexEnd,
                        self.phIndexStart,
        ...
        self.textToAvoid = text.decode("utf-8").replace(chr(160), " ")
        self.restoreDic = {}
        catchSignals()
    ...
    def allWords(self):
        ...
        return [
            x
            for x in self.dFile.read(self.wordIndexEnd - self.wordIndexStart).split(
                chr(0)
            )
            if x
        ]
    ...
    def allPh(self):
        ...
        def f(l):
            last = None
            r = []
            pos = self.phIndexStart
            for i in l:
                if (
                    re.search(r"[ -~]", i)
                    and not i in ["'a&I.'fo&Un", "'lI.@n", "'so&Un.j$"]
                    and not (i == last and i in ["'tR+e&I.si"])
                ):
                    r.append(
                        (pos, i)
                    )  # (the listed pronunciations are secondary ones that for some reason are in the list)
                if re.search(r"[ -~]", i):
                    last = i
                pos += len(i) + 1  # +1 for the \x00
            assert (
                pos == self.phIndexEnd + 1
            )  # +1 because the last \00 will result in a "" item after; the above +1 will be incorrect for that item
            return r

        return f(
            [
                x
                for x in self.dFile.read(self.phIndexEnd - self.phIndexStart).split(
                    chr(0)
                )
            ]
        )
    ...
    def usable_words(self, words_ok_to_redefine=[]):
        ...
        for word, (pos, phonemes) in zip(self.allWords(), self.allPh()):
            if not re.match("^[a-z0-9]*$", word):
                continue  # it seems words not matching this regexp are NOT used by the engine
            if not (phonemes and 32 < ord(phonemes[0]) < 127):
                continue  # better not touch those, just in case
            if word in self.textToAvoid and not word in words_ok_to_redefine:
                continue
    ...
    def check_redef(self, wordsAndPhonemes):
        ...
        for w, p in wordsAndPhonemes:
            ...
            if not ap:
                ap = self.allPh()
                sys.stderr.write(
                    "Warning: some words were already in system lexicon\nword\told\tnew\n"
                )
    ...
    def speakPhones(self, phonesList):
        ...
        os.popen(macSayCommand() + ' -v "' + self.voice + '"', "w").write(
            " ".join(d.get(w, "") for w in words)
        )
    ...
    def readWithLex(self, lex):
        ...
        tta = (
            ...
            .replace("\\u2014", " ")
            + " "
        )  # (ignore pronunciation marks 2032 and b7 that might be in the text, but still print them in textToPrint; also normalise apostrophes but not in textToPrint, and be careful with dashes as lex'ing the word after a hyphen or em-dash won't work BUT we still want to support hyphenated words IN the lexicon, so em-dashes are replaced here and hyphens are included in nonWordBefore below)
        words2, phonemes2 = (
            [],
            [],
        )  # keep only the ones actually used in the text (no point setting whole lexicon)
        nonWordBefore = (
            r"(?i)(?<=[^A-Za-z" + chr(0) + "-])"
        )  # see below for why chr(0) is included, and see comment above for why hyphen is at the end; (?i) = ignore case
        nonWordAfter = (
            r"(?=([^A-Za-z'" + chr(0x2019) + "-]|['" + chr(0x2019) + r"-][^A-Za-z]))"
        )  # followed by non-letter non-apostrophe, or followed by apostrophe non-letter (so not if followed by "'s", because the voice won't use our custom lex entry if "'s" is added to the lex'd word, TODO: automatically add "'s" versions to the lexicon via +s or +iz?) (also not if followed by hyphen-letters; hyphen before start is handled above, although TODO preceded by non-letter + hyphen might be OK)
        ttal = tta.lower()
        for ww, pp in lex:
            ww = ww.decode(
                "utf-8"
            )  # so you can add words with accents etc (in utf-8) to the lexicon
            if ww.lower() in ttal and re.search(
                nonWordBefore + re.escape(ww) + nonWordAfter, tta
            ):
                words2.append(ww)
                phonemes2.append(pp)
        ...
        if (
            "xterm" in term or term == "screen"
        ) and sys.stdout.isatty():  # we can probably underline words (inverse is more widely supported than underline, e.g. should work even on an old Linux console in case someone's using that to control an OS X server, but there might be a *lot* of words, which wouldn't be very good in inverse if user needs dark background and inverse is bright.  Unlike Annogen, we're dealing primarily with Latin letters.)
            import textwrap

            textwrap.len = lambda x: len(
                x.replace(chr(0), "").replace(chr(1), "")
            )  # a 'hack' to make (at least the 2.x implementations of) textwrap ignore our chr(0) and chr(1) markers in their calculations.  Relies on textwrap calling len().
            print(
                (
                    textwrap.fill(
                        textToPrint, stdout_width_unix(), break_on_hyphens=False
                    )
                    .encode("utf-8")
                    .replace(chr(0), "\x1b[4m")
                    .replace(chr(1), "\x1b[0m")
                    .strip()
                )
            )  # break_on_hyphens=False because we don't really want hyphenated NAMES to be split across lines, and anyway textwrap in (at least) Python 2.7 has a bug that sometimes causes a line breaks to be inserted before a syllable marker symbol like 'prime'
    ...
    def setMultiple(self, words, phonemes):
        ...
        avail.sort()
        needed.sort()  # shortest phon first
        i = 0
        wDic = {}
        iDone = set()
        mustBeAlpha = True
        ...
        for l, word, phon in needed:
            while (
                avail[i][0] < l
                or (mustBeAlpha and not re.match("[A-Za-z]", avail[i][1]))
                or i in iDone
            ):
                i += 1
                if i == len(avail):
                    if (
                        mustBeAlpha
                    ):  # desperate situation: we HAVE to use the non-alphabetical slots now (ideally we should pick words that never occur at start of sentence for them, but this branch is hopefully a rare situation in practice)
                        mustBeAlpha = False
                        i = 0
                        continue
                    sys.stderr.write(
                        "Could not find enough lexicon slots!\n"
                    )  # TODO: we passed 'words' to usable_words's words_ok_to_redefine - this might not be the case if we didn't find enough slots
                    self.dFile.flush()
                    return wDic
            iDone.add(i)
            _, wSubst, pos, oldPhon = avail[i]
            i += 1
            if avail[i][2] in self.restoreDic:
                oldPhon = (
                    None
                )  # shouldn't happen if setMultiple is called only once, but might be useful for small experiments in the Python interpreter etc
            self.set(pos, phon, oldPhon)
            wDic[word] = (
                wSubst[0].upper() + wSubst[1:]
            )  # always capitalise it so it can be used at start of sentence too (TODO: copy original capitalisation of each instance instead, in case it happens to come directly after a dotted abbreviation? although if it's something that's always capitalised anyway, e.g. most names, then this won't make any difference)
        self.dFile.flush()
        return wDic
    ...
    def set(self, phPos, val, old=None):
        ...
        if old:
            assert (
                not phPos in self.restoreDic
            ), "Cannot call set() twice on same phoneme while re-specifying 'old'"
            assert len(val) <= len(old), "New phoneme is too long!"
            self.restoreDic[phPos] = old
        else:
            assert (
                phPos in self.restoreDic
            ), "Must specify old values (for restore) when setting for first time"
...
lexFormats = (
    LexFormats()
)  # at end, in case it refers to anything that was defined later
