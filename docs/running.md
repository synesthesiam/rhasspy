...
Rhasspy is designed to be run on different kinds of hardware:

* Raspberry Pi's (`armhf`/`aarch64`)
* Desktops/laptops/servers (`amd64`)

and within different software environments:
...
The easiest way to try Rhasspy is with Docker.
...
This will start Rhasspy in the background (`-d`) on port 12101 (`-p`) and give Rhasspy access to your microphone (`--device`). Any changes you make to [your profile](profiles/) will be saved to `~/.config/rhasspy`.
